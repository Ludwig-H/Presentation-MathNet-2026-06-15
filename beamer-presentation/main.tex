  ...
  \begin{center}
    \begin{tikzpicture}[scale=1.2, x=1cm, y=1cm]
      ...
      \begin{scope}[shift={(3.2,0)}]
        ...
        \node[spin] (b) at (1.2,0) {$+$};
        \node[spin] (c) at (0.6,1.04) {$+$};
        \draw[posedge, line width=2pt, draw=inria-2024-rouge!80!black] (a)--(b);
        \draw[posedge] (b)--(c);
        \draw[posedge] (c)--(a);
      ...
      \begin{scope}[shift={(6.4,0)}]
        ...
        \node[spin] (c) at (0.6,1.04) {$+$};
        \draw[posedge, line width=2pt, draw=inria-2024-rouge!80!black] (a)--(b);
        \draw[posedge, line width=2pt, draw=inria-2024-rouge!80!black] (b)--(c);
        \draw[posedge, line width=2pt, draw=inria-2024-rouge!80!black] (c)--(a);
...
\begin{frame}{General Swendsen--Wang Cluster Dynamics}
  \begin{definition}[Local Reversibility]
    Let $P_b^{\sigma \to \sigma'}(\omega_b) = P_b^\sigma(\omega_b)$ if $P_b^{\sigma'}(\omega_b) > 0$, and $0$ otherwise.
  ...
  \begin{theorem}[Gibbs Invariance]
...
\begin{frame}{Percolation as a Necessary Condition for Recovery}
  \begin{theorem}[Percolation Bound on Overlap \textcolor{inria-2024-rouge!80!black}{\cite{BibiInformationInference}}]
    Under the uniform community prior $\mu_0$ (baseline overlap $1/K$), if the frozen clusters are recolored independently and uniformly, then for any algorithm $\tau_n$ and any $\eta > 0$:
    \begin{equation*}
        \lim_{n\to\infty} \mathbb{P}\left[ \ov_n(\Sigma_n, \tau_n) \ge \frac{1}{K} + \frac{K-1}{K}(\theta^{\max} + \eta) \right] = 0
    \end{equation*}
    where $\theta^{\max}$ is the asymptotic fraction of nodes in giant frozen components.
  \end{theorem}
  
  \begin{corollary}{Percolation is Necessary}
    In the absence of percolation ($\theta^{\max} = 0$), the maximum possible overlap is bounded by $1/K$ (random guess):
    $\theta^{\max} = 0 \implies \text{Weak (partial) recovery is impossible}$.
    Hence, \textbf{percolation of the cluster dynamics is a necessary condition for weak recovery}.
  \end{corollary}

  \begin{block}{Key Intuition}
    Small (finite) clusters are recolored independently, washing out their contribution to the global alignment. Only giant (macroscopic) components can carry global community information.
  \end{block}
\end{frame}

\begin{frame}[shrink]{Coupling \& the \textsc{Sankararaman}--\textsc{Baccelli} Bound}
