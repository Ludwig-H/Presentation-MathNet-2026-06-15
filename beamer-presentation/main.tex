...
\begin{frame}{General \textsc{Swendsen}--\textsc{Wang} Cluster Dynamics}
  \begin{block}{General Framework \textcolor{inria-2024-rouge!80!black}{\cite{BibiInformationInference}}}
    Suppose the Hamiltonian decomposes as: $U(\sigma) = U_0(\sigma) + \sum_{b \in \mathcal{B}} U_b(\sigma)$, where each $b \in \mathcal{B}$ is a \textbf{bond} (e.g. edge, triangle, clique).
    \begin{enumerate}
  ...
  \begin{block}{\textsc{Markov} Transition-Adapted Distance}
    \begin{itemize}
      ...
      \item For classical \textsc{Swendsen}--\textsc{Wang} (edges), this corresponds to the \textbf{symmetric difference} (the sum of edge weights satisfied by exactly one of the configurations):
        $d_{\mathrm{sym}}(\sigma, \sigma') = \sum_{e=\{i,j\}} |W_e| \left| \ind{\sigma \text{ satisfies } e} - \ind{\sigma' \text{ satisfies } e} \right|$.
      \item This would also establish a link to the \textbf{spectral analysis of the graph} (e.g. via Laplacian eigenvalues or mixing rates).
    \end{itemize}
  \end{block}
\end{frame}
