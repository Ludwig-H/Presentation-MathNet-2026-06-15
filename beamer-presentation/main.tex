...
\begin{frame}{The \textsc{Sankararaman}--\textsc{Baccelli} Model (GSBM)}
  \begin{block}{Model Definition \cite{SankararamanBaccelli2018}}
    \begin{itemize}
      \item \textbf{Spatial Nodes}: Let $\mathcal{X}$ be a homogeneous \textsc{Poisson} point process in $\mathbb{R}^d$ \cite{BB}. We restrict $\mathcal{X}$ to a window of volume $n$, and $V_n$ is the set of points in this window. Let $X_i$ be the position of node $i \in V_n$.
      \item \textbf{Latent Communities}: True assignments $\Sigma \in \{+1, -1\}^{V_n}$ chosen uniformly at random ($K=2$).
      \item \textbf{Geometry-Dependent Overlaps}: For each potential edge $e=\{i,j\}$ at distance $r_e = \|X_i - X_j\|$, the connection is present in the observed graph $H$ with probability:
        \begin{itemize}
          \item $f_{\mathrm{in}}(r_e)$ if $\Sigma_i = \Sigma_j$ (same community / attraction)
          \item $f_{\mathrm{out}}(r_e)$ if $\Sigma_i \neq \Sigma_j$ (different communities / repulsion)
        \end{itemize}
        where $f_{\mathrm{in}}(r) \ge f_{\mathrm{out}}(r)$ are known connecting functions.
    \end{itemize}
  \end{block}
  \begin{block}{Goal: Weak Recovery}
    \begin{itemize}
      \item Recover the true assignments $\Sigma$ up to label permutation strictly better than random guessing.
      \item Formulated via the \textbf{Community Overlap}:
        ...
        \end{equation*}
      \item We want an algorithm $\tau$ achieving $\mathbb{E}[\ov_n(\Sigma, \tau)] \ge \frac{1}{2} + \eta$ for some $\eta > 0$ as $n \to \infty$.
    \end{itemize}
  \end{block}
\end{frame}
