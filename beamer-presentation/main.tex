    ...
    \begin{itemize}
      ...
      \item The posterior $\mathbb{P}(\Sigma \mid H, X)$ is exactly a \textbf{Signed \textsc{Gibbs} measure} with weights:
        \begin{equation*}
            W_e(H)=
            \begin{cases}
                \displaystyle \log\left(\frac{f_{\mathrm{in}}(r_e)}{f_{\mathrm{out}}(r_e)}\right) > 0 & \text{if } e\in H \text{ (observed overlap / attraction } \in F\text{),}\\[2ex]
                \displaystyle -\log\left(\frac{1-f_{\mathrm{out}}(r_e)}{1-f_{\mathrm{in}}(r_e)}\right) < 0 & \text{if } e\notin H \text{ (absent overlap / repulsion } \in \tilde{F}\text{).}
            \end{cases}
        \end{equation*}
    \end{itemize}
  \end{block}
