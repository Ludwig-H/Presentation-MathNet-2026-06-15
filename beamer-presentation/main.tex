...
\begin{frame}{Coupling \& the \textsc{Sankararaman}--\textsc{Baccelli} Bound}
  \begin{block}{One Step of Classical Edge-by-Edge Swendsen--Wang}
    ...
    \begin{itemize}
      \item \textbf{Case 1 ($\Sigma_i = \Sigma_j$)}: $\mathbb{P}(\text{frozen}) = f_{\mathrm{in}}(r_e) \left(1 - e^{-|W_e|}\right) = f_{\mathrm{in}}(r_e) - f_{\mathrm{out}}(r_e)$
      \item \textbf{Case 2 ($\Sigma_i \neq \Sigma_j$)}: $\mathbb{P}(\text{frozen}) = (1 - f_{\mathrm{out}}(r_e)) \left(1 - e^{-|W_e|}\right) = f_{\mathrm{in}}(r_e) - f_{\mathrm{out}}(r_e)$
    \end{itemize}
  \end{block}
