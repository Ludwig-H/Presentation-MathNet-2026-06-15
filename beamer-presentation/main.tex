  ...
  \begin{tikzpicture}[scale=0.85, x=1.0cm, y=1.2cm, font=\footnotesize]
    ...
    \ifnum#1=2\relax
      \node[scale=0.65,chapbox, align=left, fill=white] at (0,1.75){
        \begin{tabular}{ll}
          \tikz[baseline=-0.5ex]{\draw[line width=5pt, gray!45, line cap=round] (0,0) -- (0.6,0);} & DNA Read
        \end{tabular}
      };
    \fi
    \ifnum#1>2\relax
...
\begin{frame}{Haplotype Assembly: Graphical Example}
  \centering
  \only<1>{%
  ...
  \only<3>{%
    \begin{block}{3. Modeling as a Signed Graph}
      Nodes represent reads, and edges represent overlaps. Edge signs indicate match ($+$) or mismatch ($-$).
    \end{block}
    ...
    \begin{itemize}
      \item The log-likelihood of observation $H$ under labels $\sigma$ translates into signed weights:
        \begin{equation*}
            ...
            \end{cases}
        \end{equation*}
      \item The posterior distribution is exactly a \textbf{Signed \textsc{Gibbs} measure}.
    \end{itemize}
  \end{block}
\end{frame}

\begin{frame}{The Geometric Stochastic Block Model (GSBM)}
  \begin{block}{\textsc{Sankararaman} \& \textsc{Baccelli} (SODA 2018) \cite{SankararamanBaccelli2018}}
    \begin{itemize}
      \item Nodes $V_n$ have spatial positions in $\mathbb{R}^d$ (e.g., \textsc{Poisson} process).
