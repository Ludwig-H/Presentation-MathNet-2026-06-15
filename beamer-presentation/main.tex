...
\begin{frame}{Motivation: Signed Community Detection}
  ...
  \begin{block}{The Geometric Challenge}
    \begin{itemize}
      \item Nodes are embedded in space (\emph{e.g.}, genomic positions on $\mathbb{Z}$ or $\mathbb{R}^d$).
...
\begin{frame}{\textsc{Gibbs} Model on Signed Graphs}
  \begin{block}{Attractive and Repulsive Edges}
    ...
    \begin{itemize}
      \item $F$: \textbf{Attractive} (ferromagnetic) edges, with weight $W_e > 0$
      \item $\tilde{F}$: \textbf{Repulsive} (antiferromagnetic) edges, with weight $W_e < 0$
    \end{itemize}
  \end{block}
...
\begin{frame}{From GSBM to the \textsc{Gibbs} Posterior}
  \begin{block}{Conditioning on the Point Cloud}
    \begin{itemize}
      \item With fixed spatial positions $X$, all pairwise distances $r_e = \|X_i - X_j\|$ are known.
      \item Under a uniform prior on $\Sigma$, the \textbf{posterior} distribution given the observed graph $H$ is:
        \begin{equation*}
            \alt<1>{\mathbb{P}(\Sigma = \sigma \mid H, X)}{\textcolor{red}{\boldsymbol{\mathbb{P}(\Sigma = \sigma \mid H, X) = \mu(\sigma)}}} \mathrel{\propto} \mathbb{P}(H \mid X, \sigma)
        \end{equation*}
    \end{itemize}
  \end{block}
  \begin{block}{Log-Likelihood as Signed \textsc{Gibbs} Energy}
    \begin{itemize}
      \item Choosing weights $W_e(H)$ as below:
        \begin{equation*}
            W_e(H)=
            \begin{cases}
                \displaystyle \log\left(\frac{f_{\mathrm{in}}(r_e)}{f_{\mathrm{out}}(r_e)}\right) > 0 & \text{if } e\in H \text{ (attractive } \in F\text{),}\\[2ex]
                \displaystyle -\log\left(\frac{1-f_{\mathrm{out}}(r_e)}{1-f_{\mathrm{in}}(r_e)}\right) < 0 & \text{if } e\notin H \text{ (repulsive } \in \tilde{F}\text{).}
            \end{cases}
        \end{equation*}
      \item ... yields the exact correspondence:
        \begin{equation*}
...
\begin{frame}{The Percolation Obstruction}
  \begin{block}{Necessary Condition for Weak Recovery \cite{SankararamanBaccelli2018}}
    \begin{itemize}
      \item Fragmented graphs (small, isolated components) prevent global reconstruction.
...
\begin{frame}{Open Questions}
  \begin{block}{Limits of the One-Replica Coupling}
    \begin{itemize}
      \item Our Bayesian framework combined with MCMC cluster dynamics allows us to construct \textbf{one} replica $\Sigma_n^{(1)}$.
      \item This only yields a bound (a \textbf{necessary condition} for weak recovery).
    \end{itemize}
  \end{block}
  
  \begin{block}{Towards a Necessary and Sufficient Condition}
    \begin{itemize}
      \item To achieve equality (a \textbf{necessary and sufficient condition}), a framework involving \textbf{two replicas} would be required.
      \item How can we define and analyze such a two-replica coupling under MCMC dynamics?
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}{Towards a Practical Community Detection Algorithm}
  \begin{block}{Limits of Direct \textsc{Gibbs} Sampling}
    \begin{itemize}
      \item Empirically, recovering communities by sampling directly from the \textsc{Gibbs} distribution does not perform well.
      \item This is likely because the \textbf{mixing time} of the \textsc{Markov} chain is too large.
    \end{itemize}
  \end{block}

  \begin{block}{Estimating the Correlation Matrix}
    \begin{itemize}
      \item Constructing an estimator of the correlation matrix:
        $$\mathbb{P}[\ind{\sigma_i = \sigma_j}]$$
        gives significantly better results.
      \item \textbf{Future work}: It would be highly relevant to theoretically study why this approach is much more robust.
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}{Distance Adapted to Cluster Dynamics}
  \begin{block}{\textsc{Hamming} Similarity on the Quotient Space}
    \begin{itemize}
      \item The standard overlap is essentially a \textsc{Hamming} similarity on the configuration space quotiented by label permutations:
  ...
  \begin{block}{\textsc{Markov} Transition-Adapted Distance}
    \begin{itemize}
      \item We should consider a distance that better reflects the transitions of the \textsc{Markov} cluster dynamics.
      \item For classical \textsc{Swendsen}--\textsc{Wang} (edges), this corresponds to the \textbf{symmetric difference} (the sum of edge weights satisfied by exactly one of the configurations):
        $$d_{\mathrm{sym}}(\sigma, \sigma') = \frac{1}{2} \sum_{e=\{i,j\}} |W_e| \left| \ind{\sigma \text{ satisfies } e} - \ind{\sigma' \text{ satisfies } e} \right|$$
      \item This would also establish a link to the \textbf{spectral analysis of the graph} (\emph{e.g.}, via Laplacian eigenvalues).
    \end{itemize}
  \end{block}
\end{frame}
