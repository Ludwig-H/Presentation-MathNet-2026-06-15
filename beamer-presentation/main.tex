...
% General macros
\newcommand{\SB}{Sankararaman--Baccelli}
...
\institute{Inria Paris, Room Paul Erdös (A115)}
\date[15 June 2026]{Monday 15 June 2026}
\author[L. Hauseux]{Louis \textsc{Hauseux}\\[3pt]
{\small Joint work with Konstantin \textsc{Avrachenkov} (Inria Sophia Antipolis)\\
and Nahuel \textsc{Soprano-Loto} (Inria Paris)}}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Motivation \& The Sankararaman--Baccelli Model}
\frame{\sectionpage}

\begin{frame}{Motivation: Haplotype Assembly}
  \begin{block}{Reconstructing Chromosomes from DNA Reads}
    \begin{itemize}
      \item Humans are diploid: they have two copies of each chromosome (homologous chromosomes).
      \item \textbf{Haplotype assembly}: Reconstructing the sequence of nucleotides for each of the two chromosomes.
      \item High-Throughput Sequencing generates short, overlapping fragments called \textbf{reads}.
    \end{itemize}
  \end{block}
  \begin{block}{Mapping to Signed Community Detection}
    \begin{itemize}
      \item \textbf{Nodes}: DNA reads, naturally mapped to genomic positions on $\mathbb{Z}$ (spatial structure).
      \item \textbf{Edges}: Overlap between reads (only overlapping reads share an edge).
      \item \textbf{Signs}:
        \begin{itemize}
          \item \textbf{Positive edge ($+$)}: Reads match at overlapping polymorphic sites (same chromosome).
          \item \textbf{Negative edge ($-$)}: Reads mismatch at polymorphic sites (different chromosomes).
        \end{itemize}
      \item \textbf{Hidden Communities}: $K=2$ partitions (Chromosome 1 vs Chromosome 2).
    \end{itemize}
  \end{block}
\end{frame}

\begin{frame}{Haplotype Assembly as a Signed Graph}
  \begin{figure}[H]
    \centering
    \begin{tikzpicture}[scale=0.85, x=1.0cm, y=1.2cm, font=\footnotesize]
      % 1. CHROMOSOME LINES (GROUND TRUTH)
      \draw[faint, dashed] (-1.5, 3.5) -- (13.5, 3.5) node[right, text=black, align=left] {\textbf{Haplotype 1}\\(Truth $\Sigma = +$)};
      \draw[faint, dashed] (-1.5, 0) -- (13.5, 0) node[right, text=black, align=left] {\textbf{Haplotype 2}\\(Truth $\Sigma = -$)};

      % 2. BACKGROUND: INTER-EDGES AND READS
      ...
      \node[scale=0.65,chapbox, align=left, fill=white] at (0,1.75){
        \begin{tabular}{ll}
          ...
          \tikz[baseline=-0.5ex]{\draw[posedge] (0,0) -- (0.6,0);} & Positive Edge $W_e > 0$  \\
          \tikz[baseline=-0.5ex]{\draw[negedge] (0,0) -- (0.6,0);} & Negative Edge $W_e < 0$ \\
...
\begin{frame}{The Noisy Channel \& Bayesian Formulation}
  \begin{block}{From Sequencing Errors to Graph Weights}
    \begin{itemize}
      \item Sequencing errors induce \textbf{frustration}: contradictions in pairwise labels.
      \item A sequencer has a known error rate $\varepsilon$.
      \item If two reads overlap at distance $r_e$, let:
        \begin{itemize}
          \item $f_{\mathrm{in}}(r_e) = \mathbb{P}(\text{overlap observed} \mid \text{same chromosome}) = 1 - \varepsilon$
          \item $f_{\mathrm{out}}(r_e) = \mathbb{P}(\text{overlap observed} \mid \text{different chromosomes}) = \varepsilon$
        \end{itemize}
    \end{itemize}
  \end{block}
  \begin{block}{Natural Mapping to Signed Weights}
    \begin{itemize}
      \item The log-likelihood of observation $H$ under labels $\sigma$ translates into signed weights:
        \begin{equation*}
            W_e(H)=
            \begin{cases}
                \displaystyle \log\left(\frac{f_{\mathrm{in}}(r_e)}{f_{\mathrm{out}}(r_e)}\right) > 0 & \text{if } e\in H \text{ (observed overlap),}\\[2ex]
                \displaystyle -\log\left(\frac{1-f_{\mathrm{out}}(r_e)}{1-f_{\mathrm{in}}(r_e)}\right) < 0 & \text{if } e\notin H \text{ (absent overlap).}
            \end{cases}
        \end{equation*}
      \item The posterior distribution is exactly a \textbf{Signed Gibbs measure}.
    \end{itemize}
  \end{block}
\end{frame}

\begin{frame}{The Geometric Stochastic Block Model (GSBM)}
  \begin{block}{Sankararaman \& Baccelli (SODA 2018) \cite{SankararamanBaccelli2018}}
    \begin{itemize}
      \item Nodes $V_n$ have spatial positions in $\mathbb{R}^d$ (e.g. Poisson Process).
      \item Latent community assignments $\Sigma \in \{+1, -1\}^{V_n}$ chosen uniformly at random.
      \item Connecting functions $f_{\mathrm{in}}, f_{\mathrm{out}}: \mathbb{R}_+ \to [0,1]$ with $f_{\mathrm{in}}(r) \ge f_{\mathrm{out}}(r)$.
      \item An edge $e=\{i,j\}$ with distance $r_e = \|x_i - x_j\|$ is present in $H$ with probability:
        \begin{itemize}
          \item $f_{\mathrm{in}}(r_e)$ if $\Sigma_i = \Sigma_j$ (intra-community)
          \item $f_{\mathrm{out}}(r_e)$ if $\Sigma_i \neq \Sigma_j$ (inter-community)
        \end{itemize}
    \end{itemize}
  \end{block}
  \begin{block}{Goal: Weak Recovery}
    \begin{itemize}
      \item Recover $\Sigma$ up to label permutation strictly better than random guessing.
      \item Formulated via the \textbf{Community Overlap}:
        \begin{equation*}
            \ov_n(\Sigma, \tau) = \max_{\pi \in \mathfrak{S}_2} \frac{1}{n} \sum_{i=1}^n \ind{\tau_i = \pi(\Sigma_i)}
        \end{equation*}
      \item Weak recovery is possible if there exists an algorithm $\tau$ achieving $\mathbb{E}[\ov_n(\Sigma, \tau)] \ge \frac{1}{2} + \eta$ for some $\eta > 0$ as $n \to \infty$.
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}{The Percolation Obstruction}
  \begin{block}{Necessary Condition for Weak Recovery \cite{SankararamanBaccelli2018}}
    ...
    \end{itemize}
  \end{block}
  \begin{theorem}[Sankararaman \& Baccelli \cite{SankararamanBaccelli2018}]
...
\begin{frame}{Posterior Resampling (Nishimori Identity)}
  \begin{block}{Replacing Truth by a Replica}
    ...
    \end{itemize}
  \end{block}
  \begin{theorem}[Posterior Resampling / Nishimori Identity \cite{Nishimori1980}]
...
\begin{frame}{Invariant Coupling}
  \begin{block}{Markov Chain Dynamics as a Source of Coupling}
    \begin{itemize}
      \item To study the overlap, we need a way to construct the posterior replica $\Sigma^{(1)}$ starting from the ground truth $\Sigma$.
      \item Let $M_{n, x, w}$ be a Markov transition kernel that leaves the posterior $\mu_{n, x, w}$ invariant.
...
\begin{frame}{MCMC Dynamics for Signed Gibbs Measures}
  \begin{block}{1. Single-Spin Dynamics (Glauber / Metropolis-Hastings)}
    \begin{itemize}
    ...
    \end{itemize}
  \end{block}
  \begin{block}{2. Swendsen--Wang (SW) Signed Cluster Dynamics \cite{SwendsenWang, EdwardsSokal}}
    \begin{itemize}
...
\begin{frame}{One Step of Signed Swendsen--Wang}
  \begin{figure}[H]
    \centering
...
\begin{frame}{Coupling \& The Sankararaman--Baccelli Bound}
...
\begin{frame}{Open Questions}
  ...
  \begin{block}{2. Multi-Community case ($K > 2$)}
    \begin{itemize}
      ...
      \item What is the role of Potts-like percolation in the coupling?
    \end{itemize}
  \end{block}
