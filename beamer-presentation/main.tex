...
\begin{frame}{High-Order Triangular Dynamics}
  \begin{block}{The Frustration Obstruction}
    \begin{itemize}
      ...
      \item \textbf{Higher-Order Correlation}: Introduce correlations on the freezing mechanism to \textbf{freeze as little as possible}.
    \end{itemize}
  \end{block}
  \begin{block}{Triangular Cluster Dynamics \textcolor{inria-2024-rouge!80!black}{\cite{BibiMonterey}}}
    \begin{itemize}
      \item \textbf{Energy Decomposition}: Distribute edge energy $U$ over triangles so that $U(\sigma) = \sum_{t \in \mathcal{T}} U_t(\sigma)$.
      \item \textbf{Freezing Mechanism}: Apply a freezing mechanism on the triangles.
      \item \textbf{Case $K=2$}: Triangles reduce to two types:
        \begin{itemize}
          \item \textbf{All-Attractive} (all edges satisfied, freezing all 3 edges or none).
          \item \textbf{All-Repulsive} (containing unsatisfied edges, freezing exactly one edge).
        \end{itemize}
    \end{itemize}
  \end{block}
\end{frame}
