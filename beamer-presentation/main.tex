...
\begin{frame}{General \textsc{Swendsen}--\textsc{Wang} Cluster Dynamics}
  ...
  \begin{theorem}[\textsc{Gibbs} Invariance \textcolor{inria-2024-rouge!80!black}{\cite{BibiInformationInference}}]
    ...
    \begin{enumerate}
      ...
      \item The condition $P_b^\sigma(\varnothing) > 0$ ensures the \textbf{irreducibility} of the Markov chain.
    \end{enumerate}
  \end{theorem}
\end{frame}
...
\begin{frame}{Coupling \& the \textsc{Sankararaman}--\textsc{Baccelli} Bound}
  ...
  \begin{corollary}[Retrieving the S\&B Bound \cite{SankararamanBaccelli2018}]
...
\begin{frame}{Strict Improvement of the S\&B bound (on the triangular grid)}
  ...
  \begin{corollary}[Strictly Better Impossibility Bound \textcolor{inria-2024-rouge!80!black}{\cite{BibiInformationInference}}]
    Since $p_c^{\mathrm{edge}} < p_c^{\triangle}$, for any $p \in [0.673\ldots, 0.7192\ldots)$, the standard S\&B bound cannot conclude (edge dynamics percolate), whereas triangular dynamics do not, proving that weak recovery is impossible.
  \end{corollary}
\end{frame}
...
\begin{frame}{Towards a Practical Community Detection Algorithm}
  \begin{block}{Limits of Direct \textsc{Gibbs} Sampling}
    \begin{itemize}
      ...
      \item This is likely because the \textbf{mixing time} of the Markov chain is too large.
    \end{itemize}
  \end{block}
