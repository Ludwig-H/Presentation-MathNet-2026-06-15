  ...
  \begin{block}{Homogeneous Model ($f_{\mathrm{in}} = p$, $f_{\mathrm{out}} = 1-p$)}
    ...
    \begin{itemize}
      \item \textbf{Edge Dynamics (Standard SW)}:
        ...
        Critical threshold: $p_c^{\mathrm{edge}} = \frac{1}{2} + \sin(\frac{\pi}{18}) \approx 0.673\ldots$.
      \item \textbf{Triangular Dynamics}:
        Frozen graph yields correlated triangle percolation.
        Critical threshold: $p_c^{\triangle} = \frac{7}{4} - \frac{\sqrt{17}}{4} \approx 0.7192\ldots$.
    \end{itemize}
  \end{block}
  \begin{corollary}[Strictly Better Impossibility Bound \textcolor{inria-2024-rouge!80!black}{\cite{BibiMonterey}}]
    Since $p_c^{\mathrm{edge}} < p_c^{\triangle}$, for any
    \begin{equation*}
        p \in [0.673\ldots, 0.7192\ldots),
    \end{equation*}
