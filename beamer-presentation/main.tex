...
\begin{frame}{The \textsc{Sankararaman}--\textsc{Baccelli} Model (GSBM)}
  \begin{block}{Model Definition \cite{SankararamanBaccelli2018}}
    \begin{itemize}
      \item \textbf{Spatial Nodes $V_n$}: Homogeneous \textsc{Poisson} point process $\mathcal{X}$ in $\mathbb{R}^d$ \cite{BB} restricted to a window of volume $n$. Let $X_i$ be the position of node $i \in V_n$.
  ...
  \begin{block}{Goal: Weak Recovery}
    \begin{itemize}
      \item Find an algorithm $\tau$ (independent of $\Sigma$) reconstructing assignments (up to label swap) strictly better than random guessing:
        \begin{equation*}
