...
\begin{frame}{Invariant Coupling}
  \begin{block}{\textsc{Markov} Chain Dynamics as a Source of Coupling}
    \begin{itemize}
      ...
      \item Running \textbf{exactly one step} of a posterior-invariant Markov kernel $M$ initialized at $\Sigma$ yields a valid replica $\Sigma^{(1)} \sim M(\Sigma, \cdot)$.
    \end{itemize}
  \end{block}
  \begin{figure}[H]
    \centering
    \begin{tikzpicture}[node distance=0.8cm and 1.5cm, scale=0.85, every node/.style={scale=0.85}]
      ...
      \draw[{Latex[length=2.4mm]}-{Latex[length=2.4mm]},very thick,dashed] (true) .. controls +(0,-1.4) and +(-1.4,0) .. node[left]{same score} (rep);
