  ...
  \begin{block}{Log-Likelihood as Signed Gibbs Energy}
    \begin{itemize}
      \item Factorizing the likelihood over all potential edges $e=\{i,j\} \in E$:
        ...
        \end{equation*}
      \item Choosing weights $W_e(H)$ as below yields the exact correspondence:
        \begin{equation*}
            W_e(H)=
            \begin{cases}
                \displaystyle \log\left(\frac{f_{\mathrm{in}}(r_e)}{f_{\mathrm{out}}(r_e)}\right) > 0 & \text{if } e\in H \text{ (observed overlap } \in F\text{),}\\[2ex]
                \displaystyle -\log\left(\frac{1-f_{\mathrm{out}}(r_e)}{1-f_{\mathrm{in}}(r_e)}\right) < 0 & \text{if } e\notin H \text{ (absent overlap } \in \tilde{F}\text{).}
            \end{cases}
        \end{equation*}
      \item The \textbf{posterior} $\mathbb{P}(\Sigma \mid H, X)$ is then exactly the signed \textsc{Gibbs} measure $\mu(\sigma)$.
    \end{itemize}
  \end{block}
\end{frame}
