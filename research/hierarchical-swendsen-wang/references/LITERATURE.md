...
- [Russo, *A note on percolation*](https://doi.org/10.1007/BF00535274) et [Seymour–Welsh, *Percolation probabilities on the square lattice*](https://doi.org/10.1016/S0167-5060(08)70509-0). Les théorèmes de croisement originaux ; cités pour l'historique, la forme robuste ci-dessus étant celle utilisée.
- [Kesten, *Scaling relations for 2D-percolation*](https://doi.org/10.1007/BF01205674). Relations d'échelle et quasi-multiplicativité des probabilités de bras — la source standard des arguments de quasi-multiplicativité invoqués par les feuilles de route annulaires (23, 27) ; aucune de ces relations n'est utilisée sans citation dans les résultats établis du dossier.
- [Grimmett–Manolescu, *Universality for bond percolation in two dimensions*](https://arxiv.org/abs/1108.2784), [DOI](https://doi.org/10.1214/11-AOP740) (et *Bond percolation on isoradial graphs*, [arXiv:1204.0505](https://arxiv.org/abs/1204.0505)). Transformation étoile-triangle et universalité de la percolation par **arêtes** sur les graphes isoradiaux, dont le réseau triangulaire à $`q_c=2\sin(\pi/18)`$ : box-crossing au point critique et égalité de certains exposants aux modèles de sites. C'est le pont qui rendrait licites, pour notre percolation par arêtes, une partie des estimées near-critical énoncées pour les sites — référence indispensable au verrou G1, à utiliser théorème par théorème (l'universalité complète des exposants d'arms reste conditionnelle).
...
### Ergodicité, Palm et distances critiques

- [Yao, *Law of large numbers for critical first-passage percolation on the
  triangular lattice*](https://arxiv.org/abs/1310.1247). Pour la FPP critique
  de sites triangulaire à poids $`0/1`$, le temps point-à-point est d'ordre
  $`\log n`$. C'est un guide pour choisir une échelle multiscalaire ; le
  résultat ne se transfère pas directement à la percolation d'arêtes du GSBM.
- [Baccelli–Haji-Mirsadeghi, *Point-Map-Probabilities of a Point Process and
  Mecke's Invariant Measure Equation*](https://arxiv.org/abs/1312.0287).
  Montre pourquoi un point-shift non bijectif peut demander une loi
  invariante différente, voire singulière par rapport à la Palm ordinaire.
  Le passage d'une fusion à son ancêtre est précisément plusieurs-vers-un.
- [Last–Thorisson, *Invariant transports of stationary random measures and
  mass-stationarity*](https://arxiv.org/abs/0906.2062). Cadre général des
  transports invariants, de la formule d'échange et de la mass-stationnarité.
  Il organise les biais de Palm, mais ne fournit ni grande déviation des
  bonnes cellules ni contrôle du tilt énergétique.

Ces références motivent la
[distance symétrique et le changement de mesure entropique](../active/35_DISTANCE_ENTROPIE_ERGODICITE.md).
La priorité est une grande déviation sous la loi ordinaire de la paire, pas la
construction préalable d'une mesure complète vue depuis l'ancêtre.
...
Dans l'échelle des horloges, avec $`t_\chi`$ défini par $`q_p(t_\chi)=(2p-1)^2`$ : la région d'**impossibilité** de la baseline est $`(2p-1)^2<q_c`$, c'est-à-dire $`t_\chi<\beta_c`$ (à $`p=0.8`$ on est dans l'autre régime : $`t_\chi=0.4312>\beta_c=0.4107`$, la borne ne s'applique pas). Dans le régime $`t_\chi>\beta_c`$ où l'information percole, la contraction de $`\Pi_{\beta_c}`$ réécrit la percolation d'information sur la sous-bande $`(\beta_c,t_\chi]`$, et non sur le graphe pur formé par toutes les arêtes de $`(\beta_c,1]`$.
...
La synthèse mathématique et les implications exactes pour la weak recovery sont centralisées dans [Bande critique et transmission de l'information](../diagnostics/07_CRITICAL_BAND_CRITERION.md).
...
Le [fichier 23](../archive/roadmaps/23_OPTIMAL_WEAK_RECOVERY_OBSTRUCTION.md)
reste l'historique conditionnel de l'ancienne obstruction locale.
