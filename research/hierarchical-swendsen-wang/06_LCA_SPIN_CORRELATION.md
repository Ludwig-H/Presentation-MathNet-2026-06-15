...
L'algèbre suivante est complète dès que la loi jointe $`\nu_O`$ de [01_MATHEMATICAL_FRAMEWORK.md](01_MATHEMATICAL_FRAMEWORK.md) est formalisée avec toutes ses conventions de censure. La seule dépendance restante est donc le point A1 de la feuille de route, pas une hypothèse de factorisation des nœuds.
...
=
x_i
+\sum_{u\in\operatorname{path}(i)}
m_u\sigma_i
\sum_{j\in\operatorname{sibling}_u(i)}
\sigma_jx_j.
$$
...
k
\ \stackrel{d}{=}\
1+\operatorname{Bin}(m-1,s_p(t)).
}
$$

Cette loi remplace $\operatorname{Bin}(m,p)$ au moment de l'ouverture. Elle est exacte dans ce cadre annealed et pour la coupe conditionnée ; elle ne supprime ni le biais dû au caractère aléatoire de la coupe $`E_u`$, ni le message $`B_u`$ des ancêtres.
...
- à $t=1$, $k=1+\operatorname{Bin}(m-1,1/2)$ et, dans le modèle local sans message d'ancêtre,
...
La feuille de route détaillée est centralisée dans [05_PROOF_ROADMAP.md](05_PROOF_ROADMAP.md). Pour la voie LCA, les quatre clôtures immédiates sont :
