...
3. **Block Math**: Place double dollar signs `$$` on their own lines. For complex multi-line math, prefer the ` ```math ` code block syntax.
4. **Dollar Signs**: Escape literal dollar signs inside math as `\$` and wrap them in `<span>$</span>` outside math on the same line.
